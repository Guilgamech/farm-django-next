...
import { RolForm } from "../form/create"
import { DeleteRolForm } from "../form/delete"

const columns: ColumnDef<TRol>[] = [
  {
    accessorKey: "name",
    header: ({ column }) => {
      return (
          ...
          <BasicMenu className="bg-white border-primary w-fit p-0"
            menu={<div className="flex flex-col gap-2 p-1">
              <FormModal
                ...
                heading={<span>Editar Tipo De Flota </span>}>
                <RolForm row={row.original} />
              </FormModal>
              <DeleteRolForm row={row.original} />
            </div>}>
...
const globalFilterFn: FilterFn<TRol> = (row, columnId, filterValue, addMeta) => {
  const { name} = row.original;

  return (name.toLowerCase().includes(String(filterValue).toLowerCase()))
}

export function DataTableRol() {
  const { rols, fetchingRols } = useRolStore()
  const data = useMemo(() => {
    return rols
  }, [rols])
  return <DataTable
