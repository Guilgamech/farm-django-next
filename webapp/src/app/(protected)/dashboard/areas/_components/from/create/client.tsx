...
import { areaValidationSchema, TArea, TAreaErrors, TAreaFormActionState } from "@/schema/area.schema"
import { useAreaStore } from "@/context/area"
import { useToast } from "@/components/ui/use-toast"

export const FormAreaClient = ({
  onFormAction,
  row
}: {
  onFormAction: (
    prevState: TAreaFormActionState,
    data: FormData
  ) => Promise<TAreaFormActionState>;
  row?: TArea
}) => {
  const { addArea, setArea } = useAreaStore()
  const id = row?.id
  const defaultRow = {
    code: row?.code ?? "",
    name: row?.name ?? "",
    total_area: typeof row?.total_area !== "undefined" ? String(row.total_area) : ""
  }
  const [state, formAction] = useFormState(onFormAction, {
    id
  });
  const { toast } = useToast()
  const form = useForm<z.infer<typeof areaValidationSchema>>({
  ...
  useEffect(() => {
    if (state.errors) {
      ...
      Object.keys(state.errors).forEach(el => {
        const field = el as keyof TAreaErrors
        form.setError(field, { type: "focus", message: issues[field] ?? "" }, { shouldFocus: false })
      })
    }
  }, [state.errors])
  useEffect(() => {
    if (state.type === "created" && state.area) {
      addArea(state.area)
      form.reset()
    }
    if (id && state.type === "edited" && state.area) {
      setArea({ id, data: state.area })
    }
  }, [state.type, state.area])
  useEffect(() => {
    if (state.toast) {
      toast({
    ...
    >
      ...
      <div className="mb-4">
        <FormField
          ...
          render={({ field }) => (
            <FormItem onBlur={() => form.clearErrors("root")}>
              <FormLabel className="block text-sm font-medium text-gray-700 mb-1">Área Total</FormLabel>
              <FormControl>
                <Input type="text" className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" {...field} />
              </FormControl>
              <FormMessage />
