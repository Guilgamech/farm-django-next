...
const statusOptions = [{ id: 'deteriorado', name: 'Deteriorado' }, { id: 'optimo', name: 'Óptimo' }]

export const WrapperPrintFlotasReport = () => {
  const { flotas, print, setPrint } = useFlotaPrintStore();
  ...
  const filteredFlotas = selectedStatus 
    ? flotas.filter(flota => flota.status === selectedStatus) 
    : flotas;
  ...
  return <CardTable>
    <div ref={refPrintContainer} className="w-full p-4">
      <div className="w-full flex flex-wrap justify-between gap-6 mt-5 px-5 py-8">
        ...
        <h3 className="text-xl">Reporte: Estado de las Flotas de la empresa </h3>
        </div>
      ...
      {filteredFlotas.length > 0 ? (
        ...
        <p className="text-lg mt-5 text-center font-medium">No Hay Flotas Disponibles</p>
      )}
    </div>
  </CardTable>
}