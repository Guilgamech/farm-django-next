...
import { tipocultivoValidationSchema, TTipoCultivo, TTipoCultivoErrors, TTipoCultivoFormActionState } from "@/schema/tipoc.schema"
import { useTipoCultivoStore } from "@/context/tipoc"
import { useToast } from "@/components/ui/use-toast"

export const FormTipoCultivoClient = ({
  onFormAction,
  row
}: {
  onFormAction: (
    prevState: TTipoCultivoFormActionState,
    data: FormData
  ) => Promise<TTipoCultivoFormActionState>;
  row?: TTipoCultivo
}) => {
  const { addTipoCultivo, setTipoCultivo } = useTipoCultivoStore()
  const id = row?.id
  const defaultRow = {
    name: row?.name ?? "",
  }
  ...
  const form = useForm<z.infer<typeof tipocultivoValidationSchema>>({
  ...
  useEffect(() => {
    if (state.errors) {
      ...
      Object.keys(state.errors).forEach(el => {
        const field = el as keyof TTipoCultivoErrors
        form.setError(field, { type: "focus", message: issues[field] ?? "" }, { shouldFocus: false })
      })
    }
  }, [state.errors])
  useEffect(() => {
    if (state.type === "created" && state.tipocultivo) {
      addTipoCultivo(state.tipocultivo)
      form.reset()
    }
    if (id && state.type === "edited" && state.tipocultivo) {
      setTipoCultivo({ id, data: state.tipocultivo })
    }
  }, [state.type, state.tipocultivo])
  useEffect(() => {
    if (state.toast) {
      toast({
